...
// Farben (müssen gültige HTML Farben sein)
var colors = [
    'red', 'green', 'blue', 'yellow', 'orange', 'white', 'black', 'silver'
];

// Formen (aus Sketch exportiert)
var shapes = [
...
/**
 * Stellt den Timer auf 30 Sekunden.
 */
function setBuzzerTimeout() {
    // Timer stellen
    buzzerTimeout = setTimeout(buzz, 30000);
...
/**
 * Lässt den Timer klingeln.
 */
function buzz() {
    showScreen('timeoutScreen');
    times[player] += 50000; // Strafpunkte
    tickingAudio.pause();    
...
/**
 * Screen zum Einstellen der Spielerzahl anzeigen.
 */
function setup() {
    updatePlayers();
 ...
 */
function startRound() {
    playerText.innerText = 'Spieler ' + (player + 1);
    nextPlayerText.innerText = 'Spieler ' + (player + 2);
...
/**
 * Aufgabe beantworten und Timer stoppen.
 */
function answer() {
...
/**
 * Nächster Spieler oder Ende der Runde.
 */
function nextPlayer() {
    if (++player == players) return endOfRound();
